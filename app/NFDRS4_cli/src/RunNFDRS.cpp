// RunNFDRS.cpp : This file contains the 'main' function. Program execution
...
bool fileExists(const char* fileName) {
    bool ret = false;
    if (access(fileName, 0) != -1) ret = true;
    return ret;
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        printf(
            "NFDRS4_cli takes 1 parameter: A path to a NFDRS4_cli "
            "configuration file.\n"
            "NFDRS4_cli <configFileName>\n"
            "\twhere configFileName is the complete path to a NFDRS4_cli "
            "configuration file\n\n");
        exit(1);
    }
    ...
    if (!fileExists(nfdrsInitFileName) && !fileExists(loadStateFileName)) {
        if (strlen(nfdrsInitFileName) == 0 && strlen(loadStateFileName) == 0)
            printf(
                "Error, either an NFDRSInit file or a loadStateFile must be "
                "specified.\n");
        else {
    ...
    if (strlen(wxFileName) == 0) {
        printf(
            "A wxFile must be specified in the NFDRS4_cli Configuration "
            "file\n");
        delete cfg;
        return -3;
    }
    ...
    CFW21Data FW21data;
    int status = FW21data.LoadFile(
        wxFileName, cfg->getStationID(), params.getTimeZoneOffsetHours(),
        cfg->getUseStoredOutputs() != 0 ? true : false);
    if (status != 0) {
        printf("Error loading %s as FW21 file\n", wxFileName);
        if (cfg->getUseStoredOutputs() != 0)
            printf(
                "useStoredOutputs was not zero, is %s an 'allOutputsFile' from "
                "a previous NFDRS4_cli run?\n",
                wxFileName);
        delete nfdrsCfg;
        delete cfg;
        return -5;
    ...
    FILE *allOut = NULL, *indexOut = NULL, *moistOut = NULL,
         *wxAllOut = NULL;  // , * fw21Out = NULL;
    ...
    if (allOutputsFileName && strlen(allOutputsFileName) > 0) {
        ...
        for (int fieldNum = CFW21Data::FW21_STATION;
             fieldNum < CFW21Data::FW21_TEMPC; fieldNum++) {
            if (fieldNum == 0)
                fprintf(allOut, "%s",
                        CFW21Data::GetFieldName((CFW21Data::FW21FIELDS)fieldNum)
                            .c_str());
            else
                fprintf(allOut, ",%s",
                        CFW21Data::GetFieldName((CFW21Data::FW21FIELDS)fieldNum)
    ...
    if (indexOutputsFileName && strlen(indexOutputsFileName) > 0) {
        ...
        if (!indexOut) {
            ...
            if (allOut) fclose(allOut);
            delete nfdrsCfg;
            delete cfg;
            return -3;
        }
        fprintf(indexOut, "%s,%s",
                CFW21Data::GetFieldName(CFW21Data::FW21_STATION).c_str(),
                CFW21Data::GetFieldName(CFW21Data::FW21_DATE).c_str());
        for (int f = CFW21Data::FW21_BI; f < CFW21Data::FW21_TEMPC; f++)
            fprintf(indexOut, ",%s",
                    CFW21Data::GetFieldName((CFW21Data::FW21FIELDS)f).c_str());
        fprintf(indexOut, "\n");
    }
    if (fuelMoistureOutputsFileName &&
        strlen(fuelMoistureOutputsFileName) > 0) {
        moistOut = fopen(fuelMoistureOutputsFileName, "wt");
        if (!moistOut) {
            printf("Error opening %s as output.\n",
                   fuelMoistureOutputsFileName);
            if (allOut) fclose(allOut);
            if (indexOut) fclose(indexOut);
            delete nfdrsCfg;
            delete cfg;
            return -3;
        }
        fprintf(moistOut, "%s,%s",
                CFW21Data::GetFieldName(CFW21Data::FW21_STATION).c_str(),
                CFW21Data::GetFieldName(CFW21Data::FW21_DATE).c_str());
    ...
    for (size_t r = 0; r < FW21data.GetNumRecs(); r++) {
        ...
        if (cfg->getUseStoredOutputs() != 0) {
            fw21Calc.iSetFuelMoistures(fw21Rec.GetMx1(), fw21Rec.GetMx10(),
                                       fw21Rec.GetMx100(), fw21Rec.GetMx1000(),
            ...
            fw21Calc.Update(fw21Rec.GetYear(), fw21Rec.GetMonth(),
                            fw21Rec.GetDay(), fw21Rec.GetHour(),
                            fw21Rec.GetTemp(), fw21Rec.GetRH(),
                            fw21Rec.GetPrecip(), fw21Rec.GetSolarRadiation(),
                            fw21Rec.GetWindSpeed(), fw21Rec.GetSnowFlag());
        if (cfg->getOutputInterval() == 0 ||
            (cfg->getOutputInterval() == 1 &&
             fw21Rec.GetHour() == params.getObsHour())) {
            // output to open csv files
            if (allOut) {
                fprintf(allOut,
                        "%s,%s,%.1f,%.1f,%.3f,%.1f,%d,%.1f,%d,%.1f,%d,%.10f,"
                        "%.10f,%.10f,%.10f,%.10f,%.10f,%.10f,%.2f,%.2f,%.2f,%."
                        "2f,%.10f,%d\n",
                        fw21Rec.GetStation().c_str(),
                        FW21data
                            .DateToOriginal(fw21Rec.GetDateTime(),
                                            fw21Rec.GetTimeZoneOffset())
                            .c_str(),
                        fw21Rec.GetTemp(), fw21Rec.GetRH(), fw21Rec.GetPrecip(),
                        fw21Rec.GetWindSpeed(), fw21Rec.GetWindAzimuth(),
                        fw21Rec.GetSolarRadiation(), fw21Rec.GetSnowFlag(),
                        fw21Rec.GetGustSpeed(), fw21Rec.GetGustAzimuth(),
                        fw21Calc.MC1, fw21Calc.MC10, fw21Calc.MC100,
                        fw21Calc.MC1000, fw21Calc.MCHERB, fw21Calc.MCWOOD,
                        fw21Calc.GetFuelTemperature(), fw21Calc.BI,
                        fw21Calc.ERC, fw21Calc.SC, fw21Calc.IC, fw21Calc.m_GSI,
                        fw21Calc.KBDI);
            }
            if (indexOut) {
                fprintf(indexOut, "%s,%s,%.2f,%.2f,%.2f,%.2f,%.10f,%d\n",
                        ...
                        FW21data
                            .DateToOriginal(fw21Rec.GetDateTime(),
                                            fw21Rec.GetTimeZoneOffset())
                            .c_str(),
                        fw21Calc.BI, fw21Calc.ERC, fw21Calc.SC, fw21Calc.IC,
                        fw21Calc.m_GSI, fw21Calc.KBDI);
            }
            if (moistOut) {
                fprintf(moistOut,
                        "%s,%s,%.10f,%.10f,%.10f,%.10f,%.10f,%.10f,%.10f\n",
                        fw21Rec.GetStation().c_str(),
                        FW21data
                            .DateToOriginal(fw21Rec.GetDateTime(),
                                            fw21Rec.GetTimeZoneOffset())
