...
Variable Names Used in Function Calls:
   ...
   iSCM				Fuel Model's Spread At Which All Ignitiona ->
Reportable iSite          1=Low, 2=Mid, 3=Upper iSlope30       1=30% or less,
2=greater than 30% iSlopeCls		NFDRS Slope Class (1-5) iSOW
State of Weather (0=Clear, ...) (-1 allowed in iCalcIC) iTemp
Observed Temp (F) (Can be fuel temp in iCalcIC) fWn
Fuel Model's Loading for fuel class "n" fWNDFC			Fuel Model's (or
desired) Wind Reduction Factor iWS				20-Foot
Windspeed
...
//------------------------------------------------------------------------------
/*! \class NFDRS4
    \brief Main calculator for the US National Fire Danger Rating System
...
class NFDRS4 {
   public:
    ...
    ///
    /// @param Lat The latitude of the simulation point (decimal degrees
    /// eg. 46.125)
    /// @param FuelModel Fuel Model (Fuel model character code eg. "Y", "X",
    /// etc....
    ...
    /// @param IsAnnual (Treat herbs as annuals, only allow one greenup per
    /// year)
    /// /// @return None
    ///
    NFDRS4(double Lat, char FuelModel, int SlopeClass, double AvgAnnPrecip,
           bool LT, bool Cure, bool IsAnnual);
    ~NFDRS4();
    // Member functions
    ...
    /// @param isAnnual (Treat herbs as annuals, only allow one greenup per
    /// year)
    /// @param kbdiThreshold Starting KBDI value to beging drought fuel load
    /// additions (default is 100)
    /// @param RegObsHour = 13 Regular observation hour (this is used to prompt
    /// the GSI-LFMC calculations to run once a day at this hour)
    /// @param isReinit = false (Reinitialize flag (boolean): Set to true if
    /// Init is being called mid-calculation (Not normally used)
    /// /// @return None
    ///
    void Init(double Lat, char FuelModel, int SlopeClass, double AvgAnnPrecip,
              bool LT, bool Cure, bool isAnnual, int kbdiThreshold,
              int RegObsHour = 13, bool isReinit = false);
    void Update(int Year, int Month, int Day, int Hour, int Julian, double Temp,
                double MinTemp, double MaxTemp, double RH, double MinRH,
                double PPTAmt, double pcp24, double SolarRad, double WS,
                bool SnowDay, int RegObsHr);
    void Update(int Year, int Month, int Day, int Hour, double Temp, double RH,
                double PPTAmt, double SolarRad, double WS, bool SnowDay);
    void UpdateDaily(int Year, int Month, int Day, int Julian, double Temp,
                     double MinTemp, double MaxTemp, double RH, double MinRH,
                     double pcp24, double WS, double fMC1, double fMC10,
                     double fMC100, double fMC1000, double fuelTemp,
                     bool SnowDay /* = false*/);
    bool iSetFuelModel(char cFM);
    int iSetFuelMoistures(double fMC1, double fMC10, double fMC100,
                          double fMC1000, double fMCWood, double fMCHerb,
                          double fuelTempC);
    int iCalcIndexes(int iWS, int iSlopeCls, double* fSC, double* fERC,
                     double* fBI, double* fIC, double fGSI = -999,
                     double fKBDI = -999);
    int iCalcKBDI(double fPrecipAmt, int iMaxTemp, double fCummPrecip,
                  int iYKBDI, double fAvgPrecip);
    double Cure(double fGSI = -999, double fGreenupThreshold = 0.5,
                double fGSIMax = 1.0);

    void SetGSIParams(double MaxGSI, double GreenupThreshold,
                      double TminMin = -2.0, double TminMax = 5.0,
                      double VPDMin = 900, double VPDMax = 4100,
                      double DaylMin = 36000, double DaylMax = 39600,
                      unsigned int MAPeriod = 21U, bool UseVPDAvg = false,
                      unsigned int nPrecipDays = 30, double rtPrecipMin = 0.5,
                      double rtPrecipMax = 1.5, bool UseRTPrecip = false);
    void SetHerbGSIparams(double MaxGSI, double GreenupThreshold,
                          double TminMin = -2.0, double TminMax = 5.0,
                          double VPDMin = 900, double VPDMax = 4100,
                          double DaylMin = 36000, double DaylMax = 39600,
                          unsigned int MAPeriod = 21U, bool UseVPDAvg = false,
    ...
    void SetWoodyGSIparams(double MaxGSI, double GreenupThreshold,
                           double TminMin = -2.0, double TminMax = 5.0,
                           double VPDMin = 900, double VPDMax = 4100,
                           double DaylMin = 36000, double DaylMax = 39600,
                           unsigned int MAPeriod = 21U, bool UseVPDAvg = false,
    ...
    void SetOneHourParams(double radius, double adsorptionRate,
                          double maxLocalMoisture, double stickDensity,
                          double desorptionRate);
    void SetTenHourParams(double radius, double adsorptionRate,
                          double maxLocalMoisture, double stickDensity,
                          double desorptionRate);
    void SetHundredHourParams(double radius, double adsorptionRate,
                              double maxLocalMoisture, double stickDensity,
                              double desorptionRate);
    void SetThousandHourParams(double radius, double adsorptionRate,
                               double maxLocalMoisture, double stickDensity,
                               double desorptionRate);
