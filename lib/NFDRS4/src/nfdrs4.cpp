/**
\mainpage The United States National Fire Danger Rating System, version 4
(NFDRS4)
...
CFuelModelParams::~CFuelModelParams() {}
char CFuelModelParams::getFuelModel() { return m_fuelModel; }
const char* CFuelModelParams::getDescription() { return m_description.c_str(); }
int CFuelModelParams::getSG1() { return m_SG1; }
int CFuelModelParams::getSG10() { return m_SG10; }
int CFuelModelParams::getSG100() { return m_SG100; }
int CFuelModelParams::getSG1000() { return m_SG1000; }
int CFuelModelParams::getSGHerb() { return m_SGHERB; }
int CFuelModelParams::getSGWood() { return m_SGWOOD; }
double CFuelModelParams::getL1() { return m_L1; }
double CFuelModelParams::getL10() { return m_L10; }
double CFuelModelParams::getL100() { return m_L100; }
double CFuelModelParams::getL1000() { return m_L1000; }
double CFuelModelParams::getLHerb() { return m_LHERB; }
double CFuelModelParams::getLWood() { return m_LWOOD; }
double CFuelModelParams::getDepth() { return m_DEPTH; }
int CFuelModelParams::getMXD() { return m_MXD; }
int CFuelModelParams::getHD() { return m_HD; }
int CFuelModelParams::getSCM() { return m_SCM; }
double CFuelModelParams::getLDrought() { return m_LDROUGHT; }
double CFuelModelParams::getWNDFC() { return m_WNDFC; }
void CFuelModelParams::setFuelModel(char fm) { m_fuelModel = fm; }
void CFuelModelParams::setDescription(const char* description) {
    m_description = description;
}
void CFuelModelParams::setSG1(int sg1) { m_SG1 = sg1; }
void CFuelModelParams::setSG10(int sg10) { m_SG10 = sg10; }
void CFuelModelParams::setSG100(int sg100) { m_SG100 = sg100; }
void CFuelModelParams::setSG1000(int sg1000) { m_SG1000 = sg1000; }
void CFuelModelParams::setSGHerb(int sgHerb) { m_SGHERB = sgHerb; }
void CFuelModelParams::setSGWood(int sgWood) { m_SGWOOD = sgWood; }
void CFuelModelParams::setL1(double l1) { m_L1 = l1; }
void CFuelModelParams::setL10(double l10) { m_L10 = l10; }
void CFuelModelParams::setL100(double l100) { m_L100 = l100; }
void CFuelModelParams::setL1000(double l1000) { m_L1000 = l1000; }
void CFuelModelParams::setLHerb(double lHerb) { m_LHERB = lHerb; }
void CFuelModelParams::setLWood(double lWood) { m_LWOOD = lWood; }
void CFuelModelParams::setDepth(double depth) { m_DEPTH = depth; }
void CFuelModelParams::setMXD(int mxd) { m_MXD = mxd; }
void CFuelModelParams::setHD(int hd) { m_HD = hd; }
void CFuelModelParams::setSCM(int scm) { m_SCM = scm; }
void CFuelModelParams::setLDrought(double ldrought) { m_LDROUGHT = ldrought; }
void CFuelModelParams::setWNDFC(double wndfc) { m_WNDFC = wndfc; }
...
//
NFDRS4::NFDRS4(double inLat, char FuelModel, int inSlopeClass,
               double inAvgAnnPrecip, bool LT, bool Cure, bool IsAnnual) {
    CreateFuelModels();
    StartKBDI = 100;
    Init(inLat, FuelModel, inSlopeClass, inAvgAnnPrecip, LT, Cure, IsAnnual,
         100);
}

NFDRS4::~NFDRS4() {}

void NFDRS4::Init(double inLat, char iFuelModel, int inSlopeClass,
                  double inAvgAnnPrecip, bool LT, bool Cure, bool isAnnual,
                  int kbdiThreshold, int RegObsHour /* = 13*/,
                  bool isReinit /* = false*/) {
    CTA = 0.0459137;
    NFDRSVersion = 16;           // NFDRS Model Version
    ...
    if (!isReinit)  // Initialize the live fuel moisture models
    {
        ...
        HerbFM.SetLFMParameters(HerbFM.GetMaxGSI(),
                                HerbFM.GetGreenupThreshold(),
                                HerbFM.GetMinLFMVal(), HerbFM.GetMaxLFMVal());
        WoodyFM.SetLFMParameters(
            WoodyFM.GetMaxGSI(), WoodyFM.GetGreenupThreshold(),
            WoodyFM.GetMinLFMVal(), WoodyFM.GetMaxLFMVal());
    }
    // Initialize the dead fuel moisture models
    OneHourFM.initializeParameters(0.2, "One Hour");  // 1hr Dead FM model init
    TenHourFM.initializeParameters(0.64,
                                   "Ten Hour");  // 10hr Dead FM model init
    HundredHourFM.initializeParameters(
        2.0, "Hundred Hour");  // 100hr Dead FM model init
    ...
    // sets maximum stick moisture limit. Not really relevant for 100 and 1000
    // hour moistures.
    ...
    // iSetFuelModel(FuelModel);                                   // Set the
    // Fuel model
    ...
    m_GSI = 0.0;
    nConsectiveSnowDays = 0;
    if (!isReinit) iSetFuelModel(iFuelModel);
...
void NFDRS4::SetSCMax(int maxSC) { SCM = maxSC; }

int NFDRS4::GetSCMax() { return SCM; }

void NFDRS4::SetMXD(int mxd) { MXD = mxd; }

int NFDRS4::GetMXD() { return MXD; }

double NFDRS4::GetFuelTemperature() { return FuelTemperature; }
 ...
 *
 ***************************************************************************/
int NFDRS4::iSetFuelMoistures(double fMC1, double fMC10, double fMC100,
                              double fMC1000, double fMCWood, double fMCHerb,
                              double fuelTempC) {
    MC1 = fMC1;
    MC10 = fMC10;
    MC100 = fMC100;
...
int CalcJulianDay(int year, int month, int day) {
    const int DaysOfMonth[12] = {31, 28, 31, 30, 31, 30,
                                 31, 31, 30, 31, 30, 31};
    int dayOfYear = 0;
    for (int m = 0; m < month; m++) dayOfYear += DaysOfMonth[m];
    dayOfYear += day;
    if (IsLeapYear(year) && month >= 2) dayOfYear++;
    return dayOfYear;
}

// void NFDRS4::Update(int Year, int Month, int Day, int Hour, int Julian,
// double Temp, double MinTemp, double MaxTemp, double RH, double PPTAcc, double
// PPTAmt, double SolarRad, double WS, bool SnowDay, int RegObsHr)
void NFDRS4::Update(int Year, int Month, int Day, int Hour, int Julian,
                    double Temp, double MinTemp, double MaxTemp, double RH,
                    double MinRH, double PPTAmt, double pcp24, double SolarRad,
                    double WS, bool SnowDay, int RegObsHr) {
    int tJulian = CalcJulianDay(Year, Month - 1, Day);
    if (Julian != tJulian)
        printf(
            "Julain day mismatch for Year = %d, Month = %d, Day = %d, passed "
            "Julian = %d, calced Julian = %d\n",
            Year, Month, Day, Julian, tJulian);

    if (PrevYear > 0 && YesterdayJDay > 0) {
        if (Year < PrevYear || (Year > (PrevYear + 1)) ||
            (365 * (Year - PrevYear) + Julian - YesterdayJDay > 30)) {
            // reinit
            Init(Lat, FuelModel, SlopeClass, AvgPrecip, UseLoadTransfer,
                 UseCuring, HerbFM.GetIsAnnual(), KBDIThreshold, m_regObsHour,
                 true);  // , HerbFM.GetMaxGSI(), HerbFM.GetGreenupThreshold());
        }
    }
    ...
    double temp = (Temp - 32.0) * 5.0 / 9.0, rh = RH / 100.0,
           /*accprcp = PPTAcc * 2.54, */ sr = SolarRad, pptamnt = PPTAmt * 2.54;
    ...
    // Need to set the date / time of the sticks to one hour before the first
    // observation time
    if (SnowCovered) {
        neltemp = 0.;
        nelrh = 0.999;
...
#pragma omp section
        {
            OneHourFM.update(Year, Month, Day, Hour, 0, 0, neltemp, nelrh,
                             nelsr, nelppt, 0.02179999999, true);
            MC1 = MyMC1 = OneHourFM.medianRadialMoisture() * 100;
            // MC1 = MyMC1 = OneHourFM.meanWtdMoisture() * 100;
        }
#pragma omp section
        {
            TenHourFM.update(Year, Month, Day, Hour, 0, 0, neltemp, nelrh,
                             nelsr, nelppt, 0.02179999999, true);
...
#pragma omp section
        {
            HundredHourFM.update(Year, Month, Day, Hour, 0, 0, neltemp, nelrh,
                                 nelsr, nelppt, 0.02179999999, true);
...
#pragma omp section
        {
            ThousandHourFM.update(Year, Month, Day, Hour, 0, 0, neltemp, nelrh,
                                  nelsr, nelppt, 0.02179999999, true);
    ...
    if (Hour == RegObsHr)  // || num_updates==0)
    {
        ...
        //  Maybe just need to change Nelson code to accept above temp / rh
        //  scales
        // int secs = difftime(thisUtcTime.timestamp(),
        ...
        if (days > 1)             // gap, deal with it by inserting zeroes
        {
            ...
            for (int p = 0; p < pDays; p++) qPrecip.push_back(0.0);
        }
        qPrecip.push_back(pcp24);
        while (qPrecip.size() > nPrecipQueueDays) qPrecip.pop_front();

        HerbFM.Update(Temp, MaxTemp, MinTemp, RH, MinRH, Julian,
                      GetXDaysPrecipitation(HerbFM.GetNumPrecipDays()),
                      thisUtcTime.timestamp());
        WoodyFM.Update(Temp, MaxTemp, MinTemp, RH, MinRH, Julian,
                       GetXDaysPrecipitation(WoodyFM.GetNumPrecipDays()),
                       thisUtcTime.timestamp());

        m_GSI = HerbFM.CalcRunningAvgGSI();
        MCHERB = HerbFM.GetMoisture(
            nConsectiveSnowDays >= SNOWDAYS_TRIGGER ? true : false);
        MCWOOD = WoodyFM.GetMoisture(
            nConsectiveSnowDays >= SNOWDAYS_TRIGGER ? true : false);
    ...
    iSetFuelMoistures(MC1, MC10, MC100, MC1000, MCWOOD, MCHERB,
                      FuelTemperature);
...
void NFDRS4::Update(int Year, int Month, int Day, int Hour, double Temp,
                    double RH, double PPTAmt, double SolarRad, double WS,
                    bool SnowDay) {
...
#pragma omp section
    ...
    qHourlyRH.push_back(RH);
    // trim the deques
    while (qHourlyTemp.size() > 24) qHourlyTemp.pop_front();
    while (qHourlyRH.size() > 24) qHourlyRH.pop_front();
    while (qHourlyPrecip.size() > 24) qHourlyPrecip.pop_front();
    // deques OK, now figure Min/Max's and 24 hour pcp
    double MinRH = NORECORD, MinTemp = NORECORD, MaxTemp = NORECORD,
           pcp24 = 0.0;

    for (std::deque<double>::iterator it = qHourlyTemp.begin();
         it != qHourlyTemp.end(); ++it) {
        if ((*it) != NORECORD) {
            if (MaxTemp == NORECORD)
                MaxTemp = *it;
            else
    ...
    for (std::deque<double>::iterator it = qHourlyRH.begin();
         it != qHourlyRH.end(); ++it) {
        if ((*it) != NORECORD) {
            if (MinRH == NORECORD)
                MinRH = *it;
    ...
    for (std::deque<double>::iterator it = qHourlyPrecip.begin();
         it != qHourlyPrecip.end(); ++it) {
        if (*it != NORECORD) pcp24 += *it;
    }
    // Update live fuel moisture once per day
...
void NFDRS4::UpdateDaily(int Year, int Month, int Day, int Julian, double Temp,
                         double MinTemp, double MaxTemp, double RH,
                         double MinRH, double pcp24, double WS, double fMC1,
                         double fMC10, double fMC100, double fMC1000,
                         double fuelTemp, bool SnowDay) {
    if (PrevYear > 0 && YesterdayJDay > 0) {
        if (Year < PrevYear || (Year > (PrevYear + 1)) ||
            (365 * (Year - PrevYear) + Julian - YesterdayJDay > 30)) {
            // reinit
    ...
    int secs =
        thisUtcTime.timestamp() -
        lastDailyUpdateTime.timestamp();  // difftime(thisTime, lastUpdateTime);
    int days = secs / 86400;              // 86400 seconds per day
...
int NFDRS4::iCalcIndexes(int iWS, int iSlopeCls, double* fSC, double* fERC,
                         double* fBI, double* fIC, double fGSI, double fKBDI) {
    ...
    double F1E, F10E, F100E, F1000E, FHERBE, FWOODE, FDEADE, FLIVEE, WDEDNE,
        WLIVNE;
    ...
    if (tmpKBDI > KBDIThreshold) {
        ...
        W1000 =
            W1000 + (W1000 / WTOTD) * (tmpKBDI - KBDIThreshold) * DroughtUnit;
        WTOT = W1 + W10 + W100 + W1000 + WTOTL;
        fDEPTH = (WTOT - W1000) / PackingRatio;
    }

    // Herbaceous Load Transfers
    // fTemp = Cure(MCHERB, fGSI, HerbFM.GetGreenupThreshold(),
    // HerbFM.GetMaxGSI());
    if (fGSI >= 0.0) m_GSI = fGSI;
    ...
    RHOBAR = ((WTOTL * RHOL) + (WTOTD * RHOD)) /
             WTOT;             // Weighted particle density of the fuel bed
    ...
    if (WTOTLN > 0) {
        ...
        MCLFE = ((MC1 * HN1) + (MC10 * HN10) + (MC100 * HN100)) /
                (HN1 + HN10 + HN100);
    ...
    F1 = SA1 / SADEAD;  // Proportion of dead-fuel surface area in 1-hour
                        // class,used as a weighting factor for ROS calculation
    F10 =
        SA10 / SADEAD;  // Proportion of dead-fuel surface area in 10-hour
                        // class,used as a weighting factor for ROS calculation
    F100 =
        SA100 / SADEAD;  // Proportion of dead-fuel surface area in 100-hour
                         // class,used as a weighting factor for ROS calculation
    if (WTOTL <= 0) {
        FHERB = 0;
        FWOOD = 0;
    ...
    FDEAD = SADEAD /
            (SADEAD +
             SALIVE);  // Fraction of Dead Fuel Surface area to total loading
    FLIVE = SALIVE /
            (SADEAD +
             SALIVE);  // Fraction of Live Fuel Surface area to total loading
    WDEADN = (F1 * W1N) + (F10 * W10N) +
             (F100 * W100N);  // Weighted deaf-fuel loading
    ...
    // Characteristic surface area-to-volume ratio of dead fuel, surface area
    // weighted
    SGBRD = (F1 * SG1) + (F10 * SG10) + (F100 * SG100);

    // Characteristic surface area-to-volume ratio of live fuel, surface area
    // weighted.
    SGBRL = (FHERB * SGHERB) + (FWOOD * SGWOOD);

    // Characteristic surface area-to-volume ratio of fuel bed, surface area
    // weighted.
    ...
    GMAOP =
        GMAMX * pow((BETBAR / BETOP), AD) * exp(AD * (1.0 - (BETBAR / BETOP)));
    ...
    LIVRT = WTMCL / MXL;
    ETAMD =
        1.0 - 2.59 * DEDRT + 5.11 * pow(DEDRT, 2.0) - 3.52 * pow(DEDRT, 3.0);
    ETAML =
        1.0 - 2.59 * LIVRT + 5.11 * pow(LIVRT, 2.0) - 3.52 * pow(LIVRT, 3.0);
    if (ETAMD < 0) ETAMD = 0;
    if (ETAMD > 1) ETAMD = 1;
    if (ETAML < 0) ETAML = 0;
    if (ETAML > 1) ETAML = 1;
    ...
    /* HL = HD */
    IR =
        GMAOP * ((WDEADN * HD * ETASD * ETAMD) + (WLIVEN * HD * ETASL * ETAML));
    fWNDFC = WNDFC;
    ...
    HTSINK =
        RHOBED * (FDEAD * (XF1 + XF10 + XF100) + FLIVE * (XFHERB + XFWOOD));
    // cout << HTSINK << " " << RHOBED << " " << FDEAD << " " << XF1 << " " <<
    // XF10 << " " << XF100 << " " << FLIVE << " " << XFHERB << " " << XFWOOD <<
    // endl;
    *fSC = IR * ZETA * (1.0 + PHISLP + PHIWND) / HTSINK;
    ...
    GMAOPE = GMAMXE * pow((BETBAR / BETOPE), ADE) *
             exp(ADE * (1.0 - (BETBAR / BETOPE)));
    ...
    LIVRTE = WTMCLE / MXL;
    ETAMDE =
        1.0 - 2.0 * DEDRTE + 1.5 * pow(DEDRTE, 2.0) - 0.5 * pow(DEDRTE, 3.0);
    ETAMLE =
        1.0 - 2.0 * LIVRTE + 1.5 * pow(LIVRTE, 2.0) - 0.5 * pow(LIVRTE, 3.0);
    ...
    TAU = 384.0 / SGBRT;
    *fERC = 0.04 * IRE * TAU;
    // cout << F1E  << " " << SG1 << " " << F10E << " " <<  SG10 << " " << F100E
    // << " " <<SG100 << " " << F1000E << " " << SG1000 << endl; cout << FDEADE
    // << " " << SGBRDE << " " << FLIVEE << " " <<  SGBRLE << endl; cout << IRE
    // << " " << TAU << " " << GMAOPE << " " << pow(SGBRTE, 1.5) << " " <<
    // SGBRTE <<  endl; cout << BETBAR << " " << BETOPE << endl;
    *fBI = (.301 * pow((*fSC * *fERC), 0.46)) * 10.0;
    ...
    QIGN = 144.5 - (0.266 * TMPPRM) - (0.00058 * TMPPRM * TMPPRM) -
           (0.01 * TMPPRM * MC1) + 18.54 * (1.0 - exp(-0.151 * MC1)) +
           6.4 * MC1;
 ...
 *
 ************************************************************************/
int NFDRS4::iCalcKBDI(double fPrecipAmt, int iMaxTemp, double fCummPrecip,
                      int iYKBDI, double fAvgPrecip) {
    int net = 0, idq = 0;
    ...
    if (iMaxTemp > 50) {
        ...
        idq = (800.0 - xkbdi) * (0.9679 * exp((double)0.0486 * xtemp) - 8.299) *
                  0.001 / (1.0 + 10.88 * exp((double)-0.04409 * fAvgPrecip)) +
              0.5;
        KBDI = KBDI + idq;
    }
    return (KBDI);
 ...
 *
 ***************************************************************************/
// double NFDRS4::Cure(double fMCHerb, double fGSI, double fGreenupThreshold,
// double fGSIMax)
double NFDRS4::Cure(double fGSI, double fGreenupThreshold, double fGSIMax) {
    (m_GSI < fGreenupThreshold)
        ? fctCur = 1
...
void NFDRS4::SetGSIParams(double MaxGSI, double GreenupThreshold,
                          double TminMin /*= -2.0*/, double TminMax /*= 5.0*/,
                          double VPDMin /*= 900 */, double VPDMax /*= 4100 */,
                          double DaylMin /*= 36000*/,
                          double DaylMax /*= 39600*/,
                          unsigned int MAPeriod /* = 21U*/, bool UseVPDAvg,
                          unsigned int nPrecipDays /* = 30*/,
                          double rtPrecipMin /*= 0.5*/,
                          double rtPrecipMax /*= 1.5*/,
                          bool UseRTPrecip /* = false*/) {
    /// GsiFM.SetLimits(TminMin, TminMax, VPDMin, VPDMax, DaylMin, DaylMax,
    /// rtPrecipMin, rtPrecipMax);
...
void NFDRS4::SetHerbGSIparams(
    double MaxGSI, double GreenupThreshold, double TminMin /*= -2.0*/,
    double TminMax /*= 5.0*/, double VPDMin /*= 900 */,
    double VPDMax /*= 4100 */, double DaylMin /*= 36000*/,
    double DaylMax /*= 39600*/, unsigned int MAPeriod /* = 21U*/,
    bool UseVPDAvg, unsigned int nPrecipDays /* = 30*/,
    double rtPrecipMin /*= 0.5*/, double rtPrecipMax /*= 1.5*/,
    bool UseRTPrecip /* = false*/, double herbMin /*= 30.0*/,
    double herbMax /* = 250.0*/) {
    ...
    if (MCHERB < herbMin) MCHERB = herbMin;
    if (MCHERB > herbMax) MCHERB = herbMax;
}

void NFDRS4::SetWoodyGSIparams(
    double MaxGSI, double GreenupThreshold, double TminMin /*= -2.0*/,
    double TminMax /*= 5.0*/, double VPDMin /*= 900 */,
    double VPDMax /*= 4100 */, double DaylMin /*= 36000*/,
    double DaylMax /*= 39600*/, unsigned int MAPeriod /* = 21U*/,
    bool UseVPDAvg, unsigned int nPrecipDays /* = 30*/,
    double rtPrecipMin /*= 0.5*/, double rtPrecipMax /*= 1.5*/,
    bool UseRTPrecip /* = false*/, double woodyMin /*= 60.0*/,
    double woodyMax /*= 200.0*/) {
    ...
    if (MCWOOD < woodyMin) MCWOOD = woodyMin;
    if (MCWOOD > woodyMax) MCWOOD = woodyMax;
}

void NFDRS4::SetOneHourParams(double radius, double adsorptionRate,
                              double maxLocalMoisture, double stickDensity,
...
void NFDRS4::SetTenHourParams(double radius, double adsorptionRate,
                              double maxLocalMoisture, double stickDensity,
                              double desorptionRate) {
...
void NFDRS4::SetHundredHourParams(double radius, double adsorptionRate,
                                  double maxLocalMoisture, double stickDensity,
                                  double desorptionRate) {
...
void NFDRS4::SetThousandHourParams(double radius, double adsorptionRate,
                                   double maxLocalMoisture, double stickDensity,
                                   double desorptionRate) {
...
void NFDRS4::SetStartKBDI(int sKBDI) { YKBDI = KBDI = StartKBDI = sKBDI; }

int NFDRS4::GetStartKBDI() { return StartKBDI; }
...
bool NFDRS4::ReadState(string fileName) {
    ...
    if (loadStatus) return LoadState(state);
    return loadStatus;
}
...
bool NFDRS4::LoadState(NFDRS4State state) {
    ...
    Init(Lat, FuelModel, SlopeClass, AvgPrecip, UseLoadTransfer, UseCuring,
         state.herbState.m_IsAnnual, KBDIThreshold);
